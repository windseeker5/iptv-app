...
// TODO: duplicated from the TV app's SearchHistoryStore.kt, plus remove()
// for the per-pill X.
...
class SearchHistoryStore(context: Context) {
    ...
    fun remove(query: String): List<String> {
        val updated = load().filterNot { it == query }
        prefs.edit().putString(KEY_HISTORY, updated.joinToString(DELIMITER)).apply()
        return updated
    }
