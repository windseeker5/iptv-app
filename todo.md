...
- [x] BUG (found 2026-09-18 on the Google TV): pressing Home while streaming left the audio playing over the launcher (same as the Fire TV bug) - the player now pauses when the app leaves the screen and resumes at the live edge when you come back; it also stops when you leave live TV through the menu (Search/Settings etc.). Verified via the audio system: player `paused` on Home, `started` on return, gone on Search.
